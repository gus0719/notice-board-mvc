	...
	<div id="wrapper">
		<form method = "post" action = "<%=contextPath%>/MVCJSTLBoard.do">
				...
				<c:choose>
					<c:when test="${status.index < totalRecord}">
						<div id = "links">
							...
							<span onclick = "category.value = 'upd', bod_no.value = '${bodDto.bod_no}', bod_pwd.value = '${bodDto.bod_pwd}', document.forms[0].submit()">[ 수정 ]</span>
							<span onclick = "category.value = 'del', bod_no.value = '${bodDto.bod_no}', bod_pwd.value = '${bodDto.bod_pwd}', document.forms[0].submit()">[ 삭제 ]</span>
							<a href = "<%=contextPath %>/MVCJSTLBoard.do?bodCtg=pageTitleList">[ 글목록 ]</a>
							<a href = "<%=contextPath %>/MVCJSTLBoard.do?bodCtg=pageWrite">[ 게시판 쓰기 ]</a>
						</div>
						<div id="domain">
							<div class = "shortDiv">
								<div class = "titles">Writer</div>
								<input class = "shortText" type = "text" value = "${bodDto.bod_writer}" readonly>
							</div>
							<div class = "shortDiv">
								<div class = "titles">Date</div>
								<input class = "shortText" type = "text" value = "${bodDto.bod_logtime}" readonly>
							</div>
							<div class = "longDiv">
								<div class = "titles">Title</div>
								<input id = "longText" type = "text" value = "${bodDto.bod_subject}" readonly>
							</div>
							<div id = "multiText">
								<div class = "titles">Contents</div>
								<textarea rows=10 cols=40 readonly>${bodDto.bod_content}</textarea>
							</div>
						</div>
					</c:when>
					<c:otherwise/>
			...
			<div id = "bottomIdx">
				...
				if(nowBlock != 0){
				%>
					<a class = "select" href = "<%=contextPath%>/MVCJSTLBoard.do?bodCtg=pageList&nowPage=<%=(nowBlock-1)*pagePerBlock%>&nowBlock=<%=(nowBlock-1)%>">&laquo</a>
				...
				%>
					<a class = "select" href = "<%=contextPath%>/MVCJSTLBoard.do?bodCtg=pageList&nowPage=<%=idx1%>&nowBlock=<%=nowBlock%>"><%=idx1 + 1%></a>
				<%																// 인덱스 선택은 현재 블럭에서만 가능
					if(idx1 + 1 == totalPage) break;
				}
				if(nowBlock+1 < totalBlock){
				%>
					<a class = "select" href = "<%=contextPath%>/MVCJSTLBoard.do?bodCtg=pageList&nowPage=<%=(nowBlock+1)*pagePerBlock%>&nowBlock=<%=nowBlock+1%>">&raquo</a>
